...
async def collect_posts(user_client, entity, last_message_id, full=False, known_ids=frozenset()):
    """Возвращает список постов на разбор (старые -> новые).

    full=True  — весь архив канала, кроме уже сохранённых (known_ids).
    full=False — инкрементально: если канал новый (last_message_id == 0),
                 берём последние INITIAL_FETCH_LIMIT постов; иначе только то,
                 что новее last_message_id.
    """
    posts = []
    if full:
