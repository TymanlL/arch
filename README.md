...
Telegram-бот для VPS: кидаешь ему ссылку на публичный **Telegram-** или
**YouTube-канал** — он выкачивает посты/видео (для YouTube берёт субтитры),
...
Платформа определяется по ссылке: `youtube.com`/`youtu.be` → YouTube, иначе Telegram.
YouTube-канал указывай ссылкой (`https://youtube.com/@handle`), Telegram — ссылкой,
